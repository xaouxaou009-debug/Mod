...
local function Xaou_L(key, fallback)
...
function Xaou009_ACS_Mod_Main:AddBtn2Item(evt, thing, objs)
    ...
    if thing ~= nil and thing.ThingType == g_emThingType.Item then
        ...
        thing:RemoveBtnData("Multiply ×2")
        thing:AddBtnData(
            Xaou_L("main.btn.multiply", "คูน×2"),
            "res/Sprs/ui/icon_hand",
            "GameMain:GetMod('Xaou009_ACS_Mod_Main'):MultiItems(bind)",
            Xaou_L("main.tip.multiply", "เพิ่มจำนวนไอเทมเป็น 2 เท่า"),
            nil
        )

        thing:RemoveBtnData("幽淬")
        thing:RemoveBtnData("หลอมอสูร")
        thing:RemoveBtnData("Demon Refine")
        thing:AddBtnData(
            Xaou_L("main.btn.youcui", "หลอมอสูร"),
            "res/Sprs/ui/icon_hand",
            "GameMain:GetMod('Xaou009_ACS_Mod_Main'):YouCuiItems(bind)",
            Xaou_L("main.tip.youcui", "ยกระดับคุณภาพไอเทมขึ้น 1 ขั้น โดยสำเร็จแน่นอน"),
            nil
        )

        thing:RemoveBtnData("灵淬")
        thing:RemoveBtnData("หลอมจิต")
        thing:RemoveBtnData("Spirit Refine")
        thing:AddBtnData(
            Xaou_L("main.btn.lingcui", "หลอมจิต"),
            "res/Sprs/ui/icon_hand",
            "GameMain:GetMod('Xaou009_ACS_Mod_Main'):LingCuiItems(bind)",
            Xaou_L("main.tip.lingcui", "หลอมจิตไอเทม 1 ครั้ง โดยสำเร็จแน่นอน"),
            nil
        )
        ...
        if thing.IsFaBao == true then
            ...
            thing:RemoveBtnData("Heavenly Tribulation 36")
            thing:AddBtnData(
                Xaou_L("main.btn.tianjie", "ทัณฑ์สวรรค์36"),
                "res/Sprs/ui/icon_hand",
                "GameMain:GetMod('Xaou009_ACS_Mod_Main'):TianJie(bind)",
                Xaou_L("main.tip.tianjie", "ทำให้อาวุธวิเศษผ่านการชำระล้างด้วยทัณฑ์สวรรค์ 36 ชั้น"),
                nil
            )
        end
    end
...
function Xaou009_ACS_Mod_Main:AddBtn2Npc(evt, thing, objs)
    ...
    if thing ~= nil and thing.ThingType == g_emThingType.Npc then
        ...
        thing:AddBtnData(
            ...
            Xaou_L("main.tip.npc_mod", "เปิดเมนูแก้ไข NPC แบบ UI ใหม่"),
            nil
        )
