...
function XaouItemWindow:AddButton(name, text, x, y, w, h, icon)
    ...
    obj.m_title.text = tostring(text)
    pcall(function()
        obj.m_title.textFormat.size = 22
...
function XaouItemWindow:OnInit()   -- แก้ขนาดกรอบ
    ...
    self.inputSearch = self:AddInput("inputSearch", "", 230, 108, 270, 28)
    ...
    self.btnPrev = self:AddButton("btnPrev", "◀ ก่อนหน้า", 120, 515, 120, 34)
    self.pageText = self:AddLabel("txtPage", "หน้า 1 / 1", 350, 519, 150, 28)
    self.btnNext = self:AddButton("btnNext", "ถัดไป ▶", 650, 515, 120, 34)
...
function XaouItemWindow:UpdateBottomLayout()
    ...
    move(self.btnPrev, 200, 515)
    move(self.pageText, 350, 519)
    move(self.btnNext, 600, 515)
