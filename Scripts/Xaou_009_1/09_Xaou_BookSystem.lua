...
function XaouItemWindow:RefreshBookPage()
    ...
    if self.statusLine1 ~= nil then
        self.statusLine1.m_title.text = string.format(XL("status.book.line1", "คัมภีร์ : %s | ค้นพบ : %s | หมวด : %s"), tostring(allCount), tostring(#list), tostring(XA(self.bookCategory)))
    end
    if self.statusLine2 ~= nil then
        local text = XL("status.book.help", "กดคัมภีร์เพื่อเปิดหน้า XML รับคัมภีร์แบบ ItemCache")
        if keyword ~= "" then text = text .. " | " .. string.format(XL("status.keyword", "คำค้น : %s"), keyword) end
    ...
    for i = startCat, endCat do
        ...
        if cat ~= nil then
            ...
            btn.tooltips = XA("หมวด") .. ": " .. tostring(XA(cat)) .. "\n" .. XA("หน้าหมวด") .. ": " .. tostring(self.bookCatPage) .. "/" .. tostring(maxCatPage)
    ...
    btnCatPrev.tooltips = XA("หมวดก่อนหน้า")
    ...
    if self.viewMode == "grid" then
        ...
        for i = startIndex, endIndex do
            ...
            if book ~= nil then
                ...
                btn.tooltips = "ID: " .. tostring(book.id) .. "\n" .. XA("หมวด") .. ": " .. tostring(XA(book.cat)) .. "\n" .. XA("กดเพื่อเปิด รับคัมภีร์")
    ...
    if row == 0 then
        local btn = self:AddButton("btnBookEmpty", XL("message.book_no_result", "ไม่พบคัมภีร์ที่ค้นหา"), 200, 185, 520, 34, nil)
        table.insert(self.itemButtons, btn)
    end

    if self.pageText ~= nil then
        self.pageText.m_title.text = XA("หน้า") .. " " .. tostring(self.page) .. " / " .. tostring(self:GetBookMaxPage()) .. " (" .. tostring(#list) .. "/" .. tostring(allCount) .. ")"
    end
end
