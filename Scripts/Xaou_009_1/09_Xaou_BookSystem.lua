...
function XaouItemWindow:RefreshBookPage()
    ...
    for i, cat in ipairs(cats) do
        if i > maxCat then break end
        local label = tostring(cat)
        if string.len(label) > 42 then label = string.sub(label, 1, 42) .. "..." end
        local name = "btnBookCat" .. tostring(i)
        local btn = self:AddButton(name, label, menuX, menuY + (i - 1) * (menuH + menuGap), menuW, menuH, nil)
